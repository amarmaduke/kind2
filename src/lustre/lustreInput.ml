  ...
  let nodes, globals = if Flags.no_tc () then
      ...
      let tc_res =  
        ...
        ; let (const_type_decls, node_contract_src) = LH.split_program declarations in
          ...
          | None -> Res.ok (ctx, const_inlined_type_and_consts @ const_inlined_nodes_and_contracts)
          | Some ln ->
              Log.log L_trace "last node is: %a"  LA.pp_print_ident ln 
            ; Res.ok (ctx, const_inlined_type_and_consts
                      @ LH.move_node_to_last ln (const_inlined_nodes_and_contracts))) 
        ) in
      let ctx, decls = match tc_res with
      | Ok (c, d) ->
      ...
      LNG.declarations_to_nodes ctx decls
  in
    (* Name of main node *)
    let main_node = 
